...
export class NoteService {
  ...
  public loadNotes(): void {
    this.http.get<Note[]>(BASE_URL + 'notes').subscribe((notes) => {
      this.store.dispatch(new LoadNotes(notes));
    });
  }

  public addNote(note: Note) {
    this.http.post<Note>(BASE_URL + 'notes', note).subscribe((resNote) => {
      this.store.dispatch(new AddNote(resNote));
    });
  }

  public deleteNote(note: Note) {
    this.http.delete<Note>(BASE_URL + 'notes/' + note.id).subscribe(() => {
  ...
  updateNote(note: Note) {
    this.http
      .put<Note>(BASE_URL + 'notes/' + note.id, note)
      .subscribe((resNote) => {
        this.store.dispatch(new UpdateNote(resNote));
      });
  }
}
